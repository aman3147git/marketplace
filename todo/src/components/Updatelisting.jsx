...
const UpdateListing = () => {
  ...
  useEffect(()=>{
     const getttingListing=async()=>{
        
            const listingid=params.lid;
            console.log(listingid);
           const res=await fetch(`${END_POINT2}/get/${listingid}`,{credentials:'include'}); 
  ...
  const submithandler=async(e)=>{
    ...
    try {
      ...
      setError(false);
      const res=await fetch(`${END_POINT2}/update/${params.lid}`,{
        method:'POST',
  ...
  return (
    <div className='p-3 max-w-4xl mx-auto min-h-screen'>
      <h1 className='text-4xl font-bold hover:scale-110 duration-75  text-center my-8'>Update Your Listing</h1>
        ...
        <div className="flex flex-col flex-1 gap-3">
          ...
          <p className='text-red-500'>{uploaderror&&uploaderror}</p>
          {
            formdata.imageUrls.length>0 && formdata.imageUrls.map((item,index)=>(
              <div className="flex justify-between bg-gray-300" key={index}>
          ...
          <button className='bg-gradient-to-tr from-pink-500 to-violet-600 p-4 uppercase rounded-lg hover:opacity-90 text-center'>Update</button>
